...
export default function HomePage() {
  ...
  const handleClear = () => {
    setSearchVal(''); setSearchRes(null); setActiveGenre(null)
    clearTimeout(timer.current)
  }
  ...
  const handleFavToggle = (id, isFav) => {
    const upd = list => list.map(m => m.id === id ? {...m, is_favorite: isFav} : m)
    setMovies(upd); setSeries(upd)
    if (searchRes) setSearchRes(upd(searchRes))
  }
      ...
      <main className="main">
        ...
        <div className="hero" style={{
          ...
          <div className="hero-content">
            {!isSearching && hero && <h1 className="hero-title">{hero.title}</h1>}
            {!isSearching && (
              <p className="hero-subtitle">
                {hero ? hero.description.slice(0,100) + '…' : 'Unlimited Movies & Series'}
              </p>
            )}
            <div className="hero-search">
              ...
              {searchVal
                ? <button className="search-button" onClick={handleClear} style={{background:'#555'}}>
        ...
        {}
        {genres.length > 0 && (
          <div style={{ padding:'14px 60px 0', display:'flex', gap:8, flexWrap:'wrap' }}>
            {genres.map(g => (
            ...
            {(activeGenre || searchRes) && (
              <button onClick={handleClear} style={{
                padding:'5px 12px', borderRadius:20, border:'1px solid #555',
            ...
            <div className="section-header">
              <h2 className="section-title">
                {searchLoading ? t('loading') : (
                  <>
                    {searchVal
                      ? <>{t('resultsFor')} <span style={{color:'#aaa'}}>"{searchVal}"</span></>
                      : t('filteredResults')
                    }
                    {!searchLoading && (
              ...
              <button onClick={handleClear} style={{
                ...
                color:'#888', padding:'6px 14px', borderRadius:4, cursor:'pointer', fontSize:13,
              }}>{t('showAll')}</button>
            </div>
            {!searchLoading && searchRes?.length === 0 && (
              <p style={{ padding:'0 60px', color:'#555' }}>{t('nothingFound')}</p>
            )}
        ...
        <section className="section" id="movies">
          ...
          {loading ? <p style={{padding:'0 60px',color:'#444'}}>{t('loading')}</p>
            : <div className="content-grid">
                {movies.map(m => (
                  <MovieCard key={m.id} movie={m}
                    onSelect={() => navigate(`/movie/${m.id}`)}
            ...
            : <div className="coming-grid">
                {comingSoon.map(m => (
                  <div className="coming-card" key={m.id}
                       onClick={() => navigate(`/movie/${m.id}`)} style={{cursor:'pointer'}}>
